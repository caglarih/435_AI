...
class State:
    ...
    def __repr__(self):
        id = [["0" for col in range(SIZE)] for row in range(SIZE)]
        for i in range(SIZE):
            for j in range(SIZE):
                id[i][j] = str(self.view[i][j])
    ...
    def create_id(self):
        self.id = [["0" for col in range(SIZE)] for row in range(SIZE)]
        for i in range(SIZE):
            for j in range(SIZE):
                self.id[i][j] = str(self.view[i][j])
        self.id = "\n".join([" ".join(a) for a in self.id])
        ...
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
                if problem.goal_test(child.state):
                    return child
                frontier.append(deepcopy(child))
    return None
